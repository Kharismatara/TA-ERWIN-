...
import Contact from "./pendaftaran/com-daftar";

function Dash() {
  const navigate = useNavigate();

  const handleLogout = async () => {
    // 1. Hapus informasi pengguna dari localStorage
    localStorage.removeItem("user_name");
    localStorage.removeItem("user_nim");

    // 2. Hapus token akses dari Firebase
    const auth = getAuth();
    try {
      await signOut(auth);
      ...
      <div className="card-container">
        ...
        >
          ...
          <div style={{ textAlign: "center", marginTop: "30px" }}>
            <p style={{ margin: "0", fontSize: "2rem", fontWeight: "bold" }}>Alvin Sugiarto</p>
            <p style={{ margin: "0", fontSize: "1rem" }}>362055401156</p>
          </div>
        ...
        >
          <Card.Body>
            ...
            <ListGroup variant="flush">
              ...
              <ListGroup.Item className="link-item" onClick={handleLogout}>
                Logout
              </ListGroup.Item>
            </ListGroup>
          </Card.Body>
